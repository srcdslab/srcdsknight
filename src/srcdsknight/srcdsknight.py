...
def download_dependency(*, filename: str, fileextension: str, cache: bool, cachePath: str, downloadURL: str, skipFailedDownload: bool) -> bool:
...
def extract_dependency(*, linkName: str, filename: str, cache: bool, cachePath: str, packagePath: str, skipFailedExtract: bool) -> bool:
    if os.path.exists(f"{packagePath}/{linkName}") and cache:
        ...
        my_tar.extractall(f"{packagePath}/{linkName}") # specify which folder to extract to
        my_tar.close()
    return True

def copy_folder_content(*, extractPath: str, subPath: str, installPath: str, directories: Dict[str, Any]) -> None:
...
def install_dependency(*, extractPath: str, installPath: str, folders: Dict[str, Any], directories: Dict[str, Any]) -> None:
    for folderName, folderConfig in folders.items():
        if folderName == "gametype":
            copy_folder_content(extractPath=extractPath, subPath=folderConfig["path"], installPath=installPath, directories=directories)
        elif folderName == "common":
            for path in folderConfig["paths"]:
                copy_folder_content(extractPath=extractPath, subPath=path, installPath=installPath, directories=directories)
...
def install_dependencies(*, config: Dict[str, any]) -> None:
    ...
    for linkName, linkConfig in config["links"].items():
        ...
        if download_dependency(filename=filename, fileextension=fileextension, cache=cache, cachePath=cachePath, downloadURL=downloadURL, skipFailedDownload=skipFailedDownload) and extract_dependency(linkName=linkName, filename=filename, cache=cache, cachePath=cachePath, packagePath=packagePath, skipFailedExtract=skipFailedExtract):
            install_dependency(extractPath=f"{packagePath}/{linkName}", installPath=installPath, folders=folders, directories=directories)
...
def cli(config_file: str, verbose: int):
